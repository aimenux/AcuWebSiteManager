...
namespace PX.Objects.FS
{
    public static class ServiceOrderCore
    {
        ...
        {
            int? linesToPost = serviceDetails.Select().AsEnumerable().Select(_ => (PXResult<FSSODet, FSPostInfo>)_)
                                                      .Where(y => ((FSSODet)y).needToBePosted() == true && ((FSPostInfo)y).isPosted() == false)
                                                      .Count();
