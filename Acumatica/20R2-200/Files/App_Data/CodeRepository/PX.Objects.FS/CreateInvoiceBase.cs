...
namespace PX.Objects.FS
{
        ...
        where TPostLine : class, IBqlTable, IPostLine, new()
    {
        ...
        public virtual void CreateInvoiceDocument(MethodParm parm)
        {
            ...
            while (retryCount > 0)
            {
                if (processShared.InvoiceGraph.GetGraph() is SOInvoiceEntry)
                {
                    ...
                    DeallocateUnusedItems(processShared.ServiceOrderGraph, docLines, processShared.ProcessGraph is CreateInvoiceByAppointmentPost);
            ...
            if (iparm.Exception == null)
            {
                using (var ts = new PXTransactionScope())
                {
                    UpdatePostInfoAndPostDet(docLines, postBatchShared.FSPostBatchRow, processShared.PostInfoEntryGraph, processShared.CacheFSPostDet, fsCreatedDocRow, fsINCreatedDocRow);
                    ts.Complete();
                }
        ...
        public virtual void DeallocateUnusedItems(ServiceOrderEntry graph, List<DocLineExt> docLines, bool postingAppointments)
        {
            ...
            FSDeallocateProcess.DeallocateServiceOrders(graph, splitsToDeallocate);
        }
