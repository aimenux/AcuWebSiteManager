...
namespace PX.Objects.PM
{
	...
	public class ProformaEntry : PXGraph<ProformaEntry, PMProforma>, IGraphWithInitialization
	{
		...
		protected virtual void _(Events.RowSelected<PMProforma> e)
		{
			...
			PXUIFieldAttribute.SetVisibility<PMProformaProgressLine.inventoryID>(ProgressiveLines.Cache, null, PXUIVisibility.Visible);
		...
		public virtual Dictionary<TotalsKey, decimal> GetBilledRetainageToDate(DateTime? cutoffDate)
		{
			...
			foreach (ARTran tran in selectBilledRetainageByLine.Select(cutoffDate))
			{
				...
				billedByTask.Add(key, tran.CuryTranAmt.GetValueOrDefault() * ARDocType.SignAmount(tran.TranType).GetValueOrDefault(1));
			}
