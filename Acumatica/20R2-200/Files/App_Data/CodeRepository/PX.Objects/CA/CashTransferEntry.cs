...
namespace PX.Objects.CA
{
	public class CashTransferEntry : PXGraph<CashTransferEntry, CATransfer>
	{
		...
		protected virtual void CATransfer_OutAccountID_FieldVerifying(PXCache sender, PXFieldVerifyingEventArgs e)
		{
			...
			VerifyAccount<CATransfer.outAccountID>(sender, e);
			VerifySubaccount<CATransfer.outAccountID>(sender, e);
		}

		private void VerifySubaccount<Field>(PXCache sender, PXFieldVerifyingEventArgs e)
			where Field : IBqlField
		{ 
			int? cashAccountID = (int)e.NewValue;
			CashAccount cashAccount = CashAccount.Select(cashAccountID);
			Sub sub = subaccount.Select(cashAccount.SubID);
			if (sub != null && sub.Active == false)
			{
				string subCD = subaccount.Cache.GetFormatedMaskField<Sub.subCD>(sub);
				PXSetPropertyException exception = new PXSetPropertyException(Messages.SubaccountIsInactive, PXErrorLevel.Error, subCD);
				sender.RaiseExceptionHandling<Field>(e.Row, cashAccount.CashAccountCD, exception);
			}
		}

		private void VerifyAccount<Field>(PXCache sender, PXFieldVerifyingEventArgs e)
			where Field : IBqlField
		{
			int? cashAccountID = (int)e.NewValue;
			CashAccount cashAccount = CashAccount.Select(cashAccountID);
			Account currenctAccount = account.Select(cashAccount.AccountID);
		...
		protected virtual void CATransfer_InAccountID_FieldVerifying(PXCache sender, PXFieldVerifyingEventArgs e)
		{
			...
			VerifyAccount<CATransfer.inAccountID>(sender, e);
			VerifySubaccount<CATransfer.inAccountID>(sender, e);
		}
