...
namespace PX.Objects.CA
{
	...
	public class CAReleaseProcess : PXGraph<CAReleaseProcess>
	{
			...
			where TCADocument : class, ICADocument, new()
		{
			...
			using (PXTransactionScope ts = new PXTransactionScope())
			{
				...
				foreach (PXResult<CATran, CashAccount, Currency, CurrencyInfo> res in resultset)
				{
					...
					{
						caadj = CAAdj.PK.Find(this, catran.OrigRefNbr, catran.OrigTranType);
					}
		...
		private void VerifyActivityOfExpenseAccountAndSub(CAExpense charge, CashAccount expCashcacount)
		{
			Account accountFromCashAccount = glAccounts.Select(expCashcacount.AccountID);
			if (accountFromCashAccount?.Active == false)
			{
				throw new PXException(Messages.ExpenseCashAccountUsesInactiveGLAccount, expCashcacount.CashAccountCD, accountFromCashAccount.AccountCD);
			}
			...
			if (cashInTransitSub?.Active == false)
			{
				...
				throw new PXException(Messages.ExpenseCashAccountUsesInactiveSub, expCashcacount.CashAccountCD, cashInTransitSubCD);
			}
