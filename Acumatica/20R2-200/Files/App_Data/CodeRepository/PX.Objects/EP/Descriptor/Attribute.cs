...
namespace PX.TM
{
	...
	public class OwnerAttribute : AcctSub2Attribute, IPXFieldUpdatingSubscriber
	{
		...
		public class Owner : Contact
		{
			...
			[PXDBString(1, IsFixed = true, BqlField = typeof(PX.Objects.CR.Standalone.EPEmployee.status))]
		...
		public OwnerAttribute(Type workgroupType, Type search, bool validateValue = true, bool inquiryMode = false, Type[] fieldList = null, string[] headerList = null)
		{
			...
			PXSelectorAttribute attr =
				new PXSelectorAttribute(search ?? CreateSelect(workgroupType),
					...
					})
				{
					...
					DescriptionField = typeof(Owner.displayName),
			...
			if (!inquiryMode)
			{
				_Attributes.Add(new PXRestrictorAttribute(typeof(Where<Owner.employeeStatus, IsNull, Or<Owner.employeeStatus, NotEqual<BAccount.status.inactive>>>), Objects.EP.Messages.InactiveEpmloyee, typeof(Owner.acctCD), typeof(Owner.employeeStatus)));
			}
		}
	...
	/// <summary>
	/// Allows show contacts which are subordinated or wingman for current logined employee.
	/// </summary>
	/// <example>
	...
	{
		public SubordinateAndWingmenOwnerAttribute()
            ...
            if (oldValue != null)
            {
                ...
                else if (oldValue == true && approved != true)
                {
                    ...
                    if (maps.Any())
                    {
                        ...
                        {
                            PXUIFieldAttribute.SetError<Hold>(cache, e.Row, ex.Message);
                            cache.RaiseExceptionHandling<Hold>(e.Row, oldValue, ex);
                        }
                    }
                    else
                    {
