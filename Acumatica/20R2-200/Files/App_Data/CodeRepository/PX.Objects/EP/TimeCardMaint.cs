...
namespace PX.Objects.EP
{
	...
	public class TimeCardMaint : PXGraph<TimeCardMaint, EPTimeCard>, PXImportAttribute.IPXPrepareItems
	{
		...
		protected virtual void _(Events.FieldDefaulting<EPTimeCardSummaryWithInfo, EPTimeCardSummaryWithInfo.workCodeID> e)
		{
			...
			{
				PMCostCode costCode = PMCostCode.PK.Find(this, e.Row.CostCodeID);
				if (costCode != null)
				{
	...
	public class TimecardApprovalAction : EPApprovalAction<EPTimeCard, EPTimeCard.isApproved, EPTimeCard.isRejected>
    {
		...
		public override IEnumerable Approve(PXAdapter adapter)
		{
			foreach (object res in adapter.Get())
			{
				EPTimeCard row = res as EPTimeCard;
				if ( row == null)
                {
					row = ((PXResult)res)[0] as EPTimeCard;

				}

				OnBeforeApproval(row);
			}
			
			return base.Approve(adapter);
		}
