...
namespace PX.Objects.CR
{
		...
		where TWhere : IBqlWhere, new()
	{
		...
		public PXSelectJoin<CRSMEmail,
			...
			LeftJoin<EPView, 
				On<CRSMEmail.noteID, Equal<EPView.noteID>, And<CRSMEmail.ownerID, Equal<EPView.contactID>>>>>,
			Where<CRSMEmail.classID, Equal<CRActivityClass.email>,
				And2<Where<CRSMEmail.createdByID, Equal<Current<AccessInfo.userID>>,
					Or<CRSMEmail.ownerID, Equal<Current<AccessInfo.contactID>>,
					Or<CRSMEmail.ownerID, IsSubordinateOfContact<Current<AccessInfo.contactID>>,
					Or<CRSMEmail.workgroupID, IsWorkgroupOfContact<Current<AccessInfo.contactID>>>>>>, 
				And<TWhere>>>,
		...
		private PXResult<CRSMEmail, EMailAccount, EPView> GetActivityResult(CRSMEmail a)
		{
			...
			EPView v = PXSelectReadonly<EPView, 
				Where<Required<CRSMEmail.noteID>, Equal<EPView.noteID>, 
					And<Required<CRSMEmail.ownerID>, Equal<EPView.contactID>>>>.Select(this, a.NoteID, a.OwnerID);

			return new PXResult<CRSMEmail, EMailAccount, EPView>(a, acc ?? new EMailAccount{EmailAccountID = a.MailAccountID}, v ?? new EPView{NoteID = a.NoteID, ContactID = a.OwnerID});
		}
