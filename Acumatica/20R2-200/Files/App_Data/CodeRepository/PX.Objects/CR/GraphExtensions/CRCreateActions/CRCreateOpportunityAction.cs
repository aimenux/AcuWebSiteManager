		...
		where TMain : class, IBqlTable, new()
	{
		...
		public virtual IEnumerable convertToOpportunity(PXAdapter adapter)
		{
			...
			if (AskExtConvert(out bool redirect))
				...
				{
					bool? overrideContact = Documents.Cache.GetValueOriginal<Document.overrideRefContact>(Documents.Current) as bool?;

					var result = Convert(new OpportunityConversionOptions
					{
