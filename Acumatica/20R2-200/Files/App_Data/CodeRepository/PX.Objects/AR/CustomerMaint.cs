...
namespace PX.Objects.AR
{
	...
	public class CustomerMaint : PXGraph<CustomerMaint, Customer>
	{
		...
		public CustomerMaint()
		{
			...
			PXUIFieldAttribute.SetDisplayName<Customer.acctName>(BAccount.Cache, "Account Name");
		}
		...
		public virtual IEnumerable NewPayment(PXAdapter adapter)
		{
			...
			if (customer != null && customer.BAccountID > 0L)
			{
				...
				newDoc.BranchID = null;
				payEntry.Document.Cache.SetValueExt<ARInvoice.customerID>(newDoc, customer.BAccountID);
			...
			.WithCombinedTypeValidation
		{
			...
			{
				if (Base.CurrentCustomer.Cache.GetStatus(Base.CurrentCustomer.Current) == PXEntryStatus.Inserted || Base.doCopyClassSettings)
				{
