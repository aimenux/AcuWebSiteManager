...
namespace PX.Objects.AR
{
	...
	public class ARInvoiceEntry : ARDataEntryGraph<ARInvoiceEntry, ARInvoice>, PXImportAttribute.IPXPrepareItems, IGraphWithInitialization
	{
		...
		[PXUIField(DisplayName = "Credit Hold", Visible = false)]
		...
		[PXButton(CommitChanges = true), PXUIField(DisplayName = "Put on Credit Hold")]
		...
		[PXButton(CommitChanges = true), PXUIField(DisplayName = "Release from Credit Hold")]
		...
		protected virtual void ARInvoice_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			if (state.ShouldDisableHeader)
			{
				...
				payInvoice.SetEnabled(doc.OpenDoc == true && (doc.Payable == true || doc.DocType == ARDocType.CreditMemo));
			...
			else
			{
				...
				PXUIFieldAttribute.SetEnabled<ARInvoice.curyID>(cache, doc, !state.IsRetainageDocument);
