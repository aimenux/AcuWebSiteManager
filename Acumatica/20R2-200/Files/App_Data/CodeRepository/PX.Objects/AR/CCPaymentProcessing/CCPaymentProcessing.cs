	...
	public class CCPaymentProcessing
	{
		...
		public virtual bool RecordAuthorization(ICCPayment payment, TranRecordData recordData)
		{
			...
			if (tran.TransactionID == null)
			{
				...
				extTran.SaveProfile = recordData.CreateProfile;
		...
		public virtual bool RecordPriorAuthorizedCapture(ICCPayment payment, TranRecordData recordData)
		{
			...
			{
				innerTranId = this.RecordTransaction(CCTranType.PriorAuthorizedCapture, tran);
			}
		...
		public virtual bool RecordVoid(ICCPayment payment, TranRecordData recordData)
		{
			...
			int? innerTranId = this.RecordTransaction(CCTranType.Void, tran);
			if (innerTranId != null)
			{
				recordData.InnerTranId = innerTranId;
				ret = true;
		...
		public virtual bool RecordUnknown(ICCPayment payment, TranRecordData recordData)
		{
			...
			{
				innerTranId = this.RecordTransaction(CCTranType.Unknown, tran);
			}
		...
		{
			IsValidPmInstance(aPmtInfo.PMInstanceID);
			CCProcTran tran = new CCProcTran();
			tran.Copy(aPmtInfo);
			ExternalTransaction origExtTran = null;
			if (!string.IsNullOrEmpty(recordData.RefExternalTranId))
			{
				origExtTran = _repository.FindCapturedExternalTransaction(aPmtInfo.PMInstanceID, recordData.RefExternalTranId);
			}

			if (origExtTran != null && (aPmtInfo.PMInstanceID == origExtTran.PMInstanceID))
			{
				//Override Orig Doc Ref by those from orig CC Tran if any
				int? refTranNbr = _repository.GetCCProcTranByTranID(origExtTran.TransactionID).Where(i => i.TranType == CCTranTypeCode.AuthorizeAndCapture 
					|| i.TranType == CCTranTypeCode.PriorAuthorizedCapture).Select(i => i.TranNbr).FirstOrDefault();
				tran.RefTranNbr = refTranNbr;
				tran.ProcessingCenterID = origExtTran.ProcessingCenterID;
			}
			else
			{
				CCProcessingCenter procCenter = null;
				if (aPmtInfo.PMInstanceID == PaymentTranExtConstants.NewPaymentProfile)
				{
					procCenter = _repository.GetCCProcessingCenter(recordData.ProcessingCenterId);
				}
				else
				{
					procCenter = _repository.FindProcessingCenter(aPmtInfo.PMInstanceID, aPmtInfo.CuryID);
				}
			...
			ExternalTransaction extTran = new ExternalTransaction();
			tran = _repository.InsertTransaction(tran, extTran);
		...
		public virtual CCProcTran PrepeareRecord(ICCPayment payment, TranRecordData recordData)
		{
			...
			if (recordData.ExternalTranId != null)
			{
				...
				if (sTran == null && payment.PMInstanceID == PaymentTranExtConstants.NewPaymentProfile)
				{
					sTran = trans.FirstOrDefault();
				}
