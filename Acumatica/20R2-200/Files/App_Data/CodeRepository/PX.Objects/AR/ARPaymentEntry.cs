...
namespace PX.Objects.AR
{
	...
	public class ARPaymentEntry : ARDataEntryGraph<ARPaymentEntry, ARPayment>
	{
		...
		[PXMergeAttributes(Method = MergeMethod.Append)]
		[PXFormula(null, typeof(SumCalc<ARInvoice.curyPaymentTotal>))]
		protected virtual void ARAdjust_CuryAdjdAmt_CacheAttached(PXCache sender)
		{
		}

		[PXMergeAttributes(Method = MergeMethod.Append)]
		[PXFormula(null, typeof(SumCalc<ARInvoice.curyDiscAppliedAmt>))]
		protected virtual void ARAdjust_CuryAdjdDiscAmt_CacheAttached(PXCache sender)
		{
		}

		[PXMergeAttributes(Method = MergeMethod.Append)]
		[PXFormula(null, typeof(SumCalc<ARInvoice.curyBalanceWOTotal>))]
		...
		protected virtual void ARAdjust_CuryAdjgPPDAmt_FieldVerifying(PXCache sender, PXFieldVerifyingEventArgs e)
		{
			...
			if (((decimal)adj.CuryDiscBal == 0 || Math.Sign((decimal)adj.CuryDiscBal) * Math.Sign((decimal)e.NewValue) > 0) &&
		...
		public class PaymentTransaction : PaymentTransactionAcceptFormGraph<ARPaymentEntry, ARPayment>
		{
			...
			protected virtual void InputCCTransactionRowInserted(Events.RowInserted<InputCCTransaction> e)
				=> SetSyncLock(Base.Document.Current, e.Row);
			...
			protected virtual void SetSyncLock(ARPayment doc, InputCCTransaction inputData)
			{
				...
				if (doc.SaveCard == true && pmInstanceId >= 0)
				{
					doc.SaveCard = false;
				}
				...
				if (doc.SaveCard == true || inputData.TranType == TranTypeList.UKNCode)
				{
			...
			protected override void RowPersisting(Events.RowPersisting<ARPayment> e)
			{
				...
				if (payment != null && Base.IsContractBasedAPI)
				{
					InputCCTransaction inputData = apiInputCCTran.Select();
					ProcessDocWithTranOneScope(payment, inputData);
				}
			}
			...
			protected virtual void ProcessDocWithTranOneScope(ARPayment doc, InputCCTransaction inputData)
			{
				...
				if ((inputData.NeedValidation == true)
					&& !CCProcessingFeatureHelper.IsFeatureSupported(procCenter, CCProcessingFeature.TransactionGetter))
				{
					throw new PXException(Messages.ERR_GettingInformationByTranIdNotSupported, procCenterId);
				}
				...
				if (inputData != null)
				{
					ValidateRecordedInfo(inputData);
				}
			...
			protected virtual void ValidateRecordedInfo(InputCCTransaction info)
			{
				...
				if (res == null)
				{
					throw new PXException(ErrorMessages.InvalidField, nameof(InputCCTransaction.TranType));
				}
			}
			...
			protected override void RowSelected(Events.RowSelected<ARPayment> e)
			{
				...
				if (doc.DocType == ARDocType.Refund && doc.IsCCPayment == true)
				{
				...
				if (enableCCProcess && ARSetup.Current.IntegratedCCProcessing == true && !docReleased)
				{
					...
					{
						Base.release.SetEnabled(!docOnHold && docOpen && extTranState.IsSettlementDue && doc.PendingProcessing == false);
					}
					else
					{
			...
			public bool EnableCCProcess(ARPayment doc)
			{
				...
				enableCCProcess = enableCCProcess && !doc.Voided.Value /*&& !doc.Released.Value*/;
			...
			public bool CanValidate(ARPayment doc)
			{
				...
				ExternalTransactionState state = GetActiveTransactionState();
				bool canValidate = doc.Hold != true && IsDocTypePayment(doc)
					&& (CanCapture(doc) || CanAuthorize(doc) || state.IsOpenForReview 
					|| state.NeedSync || doc.PMInstanceID == PaymentTranExtConstants.NewPaymentProfile);
			...
			private bool ForceSaveCard(ARPayment payment)
			{
				...
				if (saveCustOpt == SavePaymentProfileCode.Force && pm?.PaymentType == PaymentMethodType.CreditCard
			...
			[PXUIField(DisplayName = "Void CC Payment for Reauthorization", Visible = false, MapEnableRights = PXCacheRights.Update, MapViewRights = PXCacheRights.Update)]
			[PXProcessButton]
			public virtual IEnumerable VoidCCPaymentForReAuthorization(PXAdapter adapter)
			{
				...
				PXLongOperation.StartOperation(Base, delegate
				{
					...
					foreach (ARPayment doc in list)
					{
						...
						var adapterForOnePayment = new PXAdapter(new PXView.Dummy(paymentGraph, paymentGraph.Document.View.BqlSelect, 
																new List<object>() { paymentGraph.Document.Current }));
			...
			{
				return new ARPaymentAfterProcessingManager() { ReleaseDoc = true, RaisedVoidForReAuthorization = RaisedVoidForReAuthorization};
			}
			...
			protected override ARPaymentEntry CreateNewGraph()
			{
				string docType = Base.Document.Current.DocType;
				string refNbr = Base.Document.Current.RefNbr;
				ARPaymentEntry graph = GetGraphByDocTypeRefNbr(docType, refNbr);
				return graph;
			}
			...
			private void SetVisibilityCreditCardControlsForRefund(PXCache cache, ARPayment doc)
			{
				...
				{
					bool refTranManualEntered = !string.IsNullOrEmpty(doc.RefTranExtNbr);
					bool isCCPayment = doc.IsCCPayment.GetValueOrDefault();
					PXUIFieldAttribute.SetEnabled<ARPayment.processingCenterID>(cache, doc, refTranManualEntered || isCCPayment);
					PXUIFieldAttribute.SetVisible<ARPayment.processingCenterID>(cache, doc, refTranManualEntered || isCCPayment);
					PXUIFieldAttribute.SetVisible<ARPayment.pMInstanceID>(cache, doc, !refTranManualEntered && !isCCPayment);
				}
				PXUIFieldAttribute.SetEnabled<ARPayment.pMInstanceID>(cache, doc, false);
			}
			...
			private void CheckProcessingCenter(PXCache cache, ARPayment doc)
			{
				...
				if (doc != null && doc.PMInstanceID != PaymentTranExtConstants.NewPaymentProfile)
				{
					...
					CustomerPaymentMethod cpm = repo.GetCustomerPaymentMethod(doc.PMInstanceID);
					if (cpm?.CCProcessingCenterID != null && doc.ProcessingCenterID == null)
					{
				...
				if (doc.PMInstanceID == PaymentTranExtConstants.NewPaymentProfile && pm?.PaymentType == PaymentMethodType.CreditCard
					&& pm?.IsAccountNumberRequired == true)
				{
			...
			protected virtual void FieldUpdated(Events.FieldUpdated<ARPayment.paymentMethodID> e)
			{
				...
				if (payment.DocType == ARDocType.Refund)
				{
					...
					cache.SetValueExt<ARPayment.newCard>(payment, true);
				}
				else
				{ 
					object retVal;
			...
			protected virtual void FieldUpdated(Events.FieldUpdated<ARPayment.newCard> e)
			{
				...
				if (e.ExternalCall == true && payment != null)
				{
					if (newValue == true)
					{
						...
						cache.SetDefaultExt<ARPayment.processingCenterID>(e.Row);
					...
					else
					{
						...
						cache.RaiseFieldDefaulting<ARPayment.pMInstanceID>(payment, out instance);
						cache.SetValueExt<ARPayment.pMInstanceID>(payment, instance);
						cache.SetValueExt<ARPayment.processingCenterID>(e.Row, null);
						cache.SetValueExt<ARPayment.saveCard>(payment, false);
					}
				}
			}
			...
			protected virtual void FieldUpdated(Events.FieldUpdated<ARPayment.refTranExtNbr> e)
			{
				...
				if (!string.IsNullOrEmpty(val))
				{
					...
					else
					{
						...
						{
							cache.SetValueExt<ARPayment.newCard>(payment, false);
		...
		private static ARPaymentEntry GetGraphByDocTypeRefNbr(string docType, string refNbr)
		{
			...
			paymentGraph.IgnoreNegativeOrderBal = true; // TODO: SOCreatePayment: Temporary fix ARPayment bug (AC-159389), after fix we should remove this code.
		...
		protected virtual void ARPayment_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			bool useNewCard = paymentmethod.Current?.PaymentType == PaymentMethodType.CreditCard && doc.DocType != ARDocType.Refund;
			bool newCardVisible = useNewCard && ShowCardChck(doc) && !currState.IsActive;
			...
			CustomerClass custClass = customerclass.SelectSingle();
			string saveCustOpt = custClass?.SavePaymentProfiles;
			...
			else if (enableCCProcess && (currState.IsPreAuthorized || currState.IsSettlementDue || currState.IsImportedUnknown))
			{
				PXUIFieldAttribute.SetEnabled(cache, doc, false);
				PXUIFieldAttribute.SetEnabled<ARPayment.adjDate>(cache, doc, true);
				PXUIFieldAttribute.SetEnabled<ARPayment.adjFinPeriodID>(cache, doc, true);
		...
		{
			return (doc.DocType == ARDocType.Payment || doc.DocType == ARDocType.Prepayment || doc.DocType == ARDocType.Refund) && doc.Voided == false;
		}
