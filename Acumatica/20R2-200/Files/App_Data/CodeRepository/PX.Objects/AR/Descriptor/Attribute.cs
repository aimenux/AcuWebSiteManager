...
namespace PX.Objects.AR
{
	...
	public abstract class RestrictCustomerBySource : PXRestrictorAttribute
	{
		public bool ResetCustomer;
		protected Type SourceType;
		protected Type AcctCD;

		protected RestrictCustomerBySource(Type restriction, Type source, Type acctCD)
			: base(restriction, Messages.BranchRestrictedByCustomer)
		{
			SourceType = source;
			AcctCD = acctCD;
		}
	...
	public abstract class RestrictBranchBySource : PXRestrictorAttribute
	{
		...
		protected RestrictBranchBySource(Type restriction, Type source, Type branchCD)
			: base(restriction, Messages.BranchRestrictedByCustomer)
		{
			SourceType = source;
			BranchCD = branchCD;
		...
		public override object[] GetMessageParameters(PXCache sender, object itemres, object row)
		{
			...
			var cacheCustomer = sender.Graph.Caches[BqlCommand.GetItemType(SourceType)];
			var msgCustomer = cacheCustomer.GetStateExt(PXResult.Unwrap(row, BqlCommand.GetItemType(SourceType)) ?? cacheCustomer.Current, SourceType.Name);
			result.Add(msgCustomer);
	...
	public class RestrictCustomerByUserBranches : PXRestrictorAttribute
	{
		...
		public override object[] GetMessageParameters(PXCache sender, object itemres, object row)
		{
			...
			var cacheBranch = sender.Graph.Caches[BqlCommand.GetItemType(typeof(AccessInfo.branchID))];
			var msgBranch = cacheBranch.GetStateExt(cacheBranch.Current, typeof(AccessInfo.branchID).Name);
			result.Add(msgBranch);
	...
	public class RestrictCustomerByOrganization : RestrictCustomerBySource
	{		
		public RestrictCustomerByOrganization(Type orgBAccountID = null)
			:base(BqlCommand.Compose(typeof(Where<,>),
				...
				typeof(Current<>),
				orgBAccountID), orgBAccountID, typeof(Customer.acctCD))
		{
			ResetCustomer = true;
		}
	}
	public class RestrictCustomerByBranch : RestrictCustomerBySource
	{
		public RestrictCustomerByBranch(Type branchID = null)
			: base(BqlCommand.Compose(typeof(Where<,>),
				...
				typeof(Current<>),
				branchID), source: branchID, acctCD: typeof(Customer.acctCD))
		{
			ResetCustomer = false;
		}
		...
		public RestrictCustomerByBranch(Type source, Type branchID = null)
			: base(BqlCommand.Compose(typeof(Where<,>),
				...
				typeof(RestrictByBranch<>),
				typeof(Current<>),
				branchID), source: branchID, acctCD: source)
		{
			ResetCustomer = false;
		}
	}
		...
		public RestrictBranchByCustomer(Type source, Type cOrgBAccountID = null)
			: base(BqlCommand.Compose(
				...
				typeof(Or<,>),
					...
					typeof(Equal<>),
					typeof(Zero)
				), source: source, branchCD: typeof(Branch.branchCD))
		{
		}

		protected override void SourceChanged(PXCache cache, PXFieldUpdatedEventArgs e)
		{
			SyncCustomerCache(cache, e.Row);
			base.SourceChanged(cache, e);
		}

		protected virtual void OnRowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			SyncCustomerCache(sender, e.Row);
		}

		protected void SyncCustomerCache(PXCache cache, object row)
		{
