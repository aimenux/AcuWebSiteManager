...
namespace PX.Objects.AR
{
		...
		: IBqlTable, IAdjustment, IDocumentAdjustment, IAdjustmentAmount
	{
		...
		{
			public class Payment : AR.ARPayment.PK.ForeignKeyOf<ARAdjust>.By<adjgDocType, adjgRefNbr> { }
			public class Invoice : AR.ARInvoice.PK.ForeignKeyOf<ARAdjust>.By<adjdDocType, adjdRefNbr> { }
		}
		#region Selected
	...
	public partial class ARAdjust2 : PX.Objects.AR.ARAdjust, ICreatePaymentAdjust
	{
		...
		[PXUnboundFormula(
			typeof(Switch<Case<Where<released, Equal<False>, And<paymentReleased, Equal<False>, And<isCCAuthorized, Equal<False>>>>,
				curyAdjdAmt>, decimal0>),
			typeof(SumCalc<AR.ARInvoice.curyUnreleasedPaymentAmt>),
			ForceAggregateRecalculation = true)]
		[PXUnboundFormula(
			typeof(Switch<Case<Where<released, Equal<False>, And<paymentReleased, Equal<False>, And<isCCAuthorized, Equal<True>>>>,
				curyAdjdAmt>, decimal0>),
			typeof(SumCalc<AR.ARInvoice.curyCCAuthorizedAmt>))]
		[PXUnboundFormula(
			typeof(Switch<Case<Where<released, Equal<False>, And<paymentReleased, Equal<True>>>,
				curyAdjdAmt>, decimal0>),
