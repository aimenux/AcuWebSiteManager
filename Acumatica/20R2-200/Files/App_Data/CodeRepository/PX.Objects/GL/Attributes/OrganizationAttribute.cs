	...
	public class PXSelectOrganizationTree : PXSelectBase<BranchItem>
	{
		...
		)
		{
			...
			AddOrganizations(result, null,
				PXAccess.GetOrganizations(OnlyActive, false).Where(_ => _.DeletedDatabaseRecord == false));
		...
		private void AddOrganizations(List<BranchItem> result,
			...
			foreach (var organization in organizations.Where(_ => _.DeletedDatabaseRecord == false))
				{
				var o = CreateItem(organization.BAccountID, organization.OrganizationCD, organization.OrganizationName, masterOrganization?.BAccountID);
				result.Add(o);
		...
		private BranchItem CreateItem(int? id, string cd, string name, int? parentId = null)
				{
