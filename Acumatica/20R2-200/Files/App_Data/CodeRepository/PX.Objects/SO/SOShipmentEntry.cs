...
namespace PX.Objects.SO
{
	public partial class SOShipmentEntry : PXGraph<SOShipmentEntry, SOShipment>, IGraphWithInitialization
	{
		...
		public PXAutoAction<SOShipment> initializeState;
		...
		public virtual IEnumerable EmailShipment(PXAdapter adapter) => Notification(adapter, "SHIPMENT");
		...
		[PXButton(CommitChanges = true), PXUIField(DisplayName = ShipmentActions.Messages.CreateDropshipInvoice, MapEnableRights = PXCacheRights.Select, MapViewRights = PXCacheRights.Select)]
		...
		protected virtual IEnumerable CreateDropshipInvoice(PXAdapter adapter)
		{
			...
			{
				var shipmentEntry = CreateInstance<SOShipmentEntry>();
				InvoiceList createdInvoices = new InvoiceList(shipmentEntry);
			...
			if (!String.IsNullOrEmpty(reportID) && shipments.Any())
			{
				...
				PXReportRequiredException combinedReport = shipments
					...
					.Aggregate(
						...
						(acc, elem) => PXReportRequiredException.CombineReport(acc, elem.ActualReportID, elem.Parameters));
