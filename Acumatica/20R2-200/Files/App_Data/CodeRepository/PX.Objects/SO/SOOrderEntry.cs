...
namespace PX.Objects.SO
{
	...
	public partial class SOOrderEntry : PXGraph<SOOrderEntry, SOOrder>, PXImportAttribute.IPXPrepareItems, PXImportAttribute.IPXProcess, IGraphWithInitialization
	{
		...
		public PXAutoAction<SOOrder> initializeState;
		...
		[PXButton(CommitChanges = true), PXUIField(DisplayName = "Put on Hold", MapEnableRights = PXCacheRights.Select)]
		protected virtual IEnumerable PutOnHold(PXAdapter adapter) => adapter.Get<SOOrder>();

		public PXAction<SOOrder> releaseFromHold;
		[PXButton(CommitChanges = true), PXUIField(DisplayName = "Release from Hold")]
		...
		[PXButton(CommitChanges = true), PXUIField(DisplayName = "Release from Credit Hold", MapEnableRights = PXCacheRights.Select)]
		...
		[PXButton(CommitChanges = true), PXUIField(DisplayName = "Print Sales Order/Quote", MapEnableRights = PXCacheRights.Select)]
		public virtual IEnumerable PrintSalesOrder(PXAdapter adapter) => Report(adapter.Apply(it => it.Menu = "Print Sales Order/Quote"), "SO641010");
		...
		[PXButton(CommitChanges = true), PXUIField(DisplayName = "Email Sales Order/Quote")]
		public virtual IEnumerable EmailSalesOrder(PXAdapter adapter) => Notification(adapter, "SALES ORDER");
