...
namespace PX.Objects.SO
{
	public partial class SOInvoiceEntry : ARInvoiceEntry
	{
		...
		public PXAutoAction<ARInvoice> initializeState;

		public PXAction<ARInvoice> putOnHold;
		[PXButton(CommitChanges = true), PXUIField(DisplayName = "Put on Hold", MapEnableRights = PXCacheRights.Select, MapViewRights = PXCacheRights.Select)]
		protected virtual IEnumerable PutOnHold(PXAdapter adapter) => adapter.Get();

		public PXAction<ARInvoice> releaseFromHold;
		[PXButton(CommitChanges = true), PXUIField(DisplayName = "Release from Hold", MapEnableRights = PXCacheRights.Select, MapViewRights = PXCacheRights.Select)]
		...
		public virtual void ReleaseInvoiceProc(List<ARRegister> list, bool isMassProcess)
		{
			...
			try
			{
				...
				ARDocumentRelease.ReleaseDoc(list, isMassProcess, null, null, delegate (ARRegister ardoc)
				{
					...
					SOInvoice soInvoice = this.SODocument.SelectSingle(ardoc.DocType, ardoc.RefNbr);
		...
		[PXButton(CommitChanges = true), PXUIField(DisplayName = "Post Invoice to IN")]
		...
		protected virtual IEnumerable PrintInvoice(PXAdapter adapter) => Report(adapter.Apply(it => it.Menu = "Print Invoice"), "SO643000");
		...
		protected virtual IEnumerable AREdit(PXAdapter adapter) => Report(adapter.Apply(it => it.Menu = "AR Edit"), "AR610500");
		...
		protected virtual IEnumerable EmailInvoice(PXAdapter adapter) => Notification(adapter, "SO INVOICE");
