	...
	public class POReceiptEntry : PXGraph<POReceiptEntry, POReceipt>, IGraphWithInitialization
	{
		...
		public PXSelect<
			...
			Where<POOrderS.hold, Equal<boolFalse>,
				...
				And<Where<
					Current<POSetup.changeCuryRateOnReceipt>, Equal<True>, 
					And<POOrderS.curyID, Equal<Current<POReceipt.curyID>>, 
						Or<Current<POSetup.changeCuryRateOnReceipt>, Equal<False>>>>>>>>>>>>>>>>>,
		...
		protected virtual void POReceiptLine_CuryUnitCost_FieldDefaulting(PXCache sender, PXFieldDefaultingEventArgs e)
		{
			...
			if (row.AllowEditUnitCost != true || row.ManualPrice == true)
			{
			...
			else
			{
				...
				{
					if (this.AllowChangingCuryRate() && poOriginOrder?.CuryID == Document.Current.CuryID)
					{
				...
				e.NewValue = DefaultUnitCost(sender, row, !this.AllowChangingCuryRate()) ?? 0m;
			}
		}
		...
		protected virtual void POReceiptLine_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			if (row.Released != true)
			{
				...
				bool allowChangingCost =
					isReceipt && row.AllowEditUnitCost == true && (usePOAccrual && receiptBasedBilling || !usePOAccrual && !fromPO)
		...
		protected virtual void POReceiptLine_RowInserted(PXCache sender, PXRowInsertedEventArgs e)
        {
            ...
            {
				row.CuryUnitCost = DefaultUnitCost(sender, row, !this.AllowChangingCuryRate());
            }
		...
		public virtual void CopyFromOrigReceiptLine(POReceiptLine destLine, IPOReturnLineSource srcLine)
		{
			...
			destLine.AllowEditUnitCost = false;

			destLine.UnitCost = srcLine.UnitCost;	
			destLine.TranUnitCost = srcLine.TranCostFinal / srcLine.ReceiptQty;
			decimal? srcExtCost = srcLine.ExtCost;
			destLine.ExtCost = (srcLine.ReturnedQty == 0m) ? srcExtCost
				: PXCurrencyAttribute.BaseRound(this, srcExtCost * (srcLine.ReceiptQty - srcLine.ReturnedQty) / srcLine.ReceiptQty);
			...
			//for now returns are always created in base currency, but it can be changed in future
			decimal curyUnitCost, curyExtCost, curyTranCost, curyTranUnitCost;
			PXDBCurrencyAttribute.CuryConvCury(transactions.Cache, destLine, destLine.UnitCost ?? 0m, out curyUnitCost);
			destLine.CuryUnitCost = curyUnitCost;
			PXDBCurrencyAttribute.CuryConvCury(transactions.Cache, destLine, destLine.ExtCost ?? 0m, out curyExtCost);
			destLine.CuryExtCost = curyExtCost;
			...
			PXDBCurrencyAttribute.CuryConvCury(transactions.Cache, destLine, destLine.TranUnitCost ?? 0m, out curyTranUnitCost);
		...
		public virtual void CalculateAmountFromPOLine(POReceiptLine aDest, POLine aSrc, POOrder order)
		{
			...
			if (this.AllowChangingCuryRate() && order?.CuryID == Document.Current.CuryID)
			{
				...
				aDest.CuryTranCost = PXCurrencyAttribute.Round(transactions.Cache, aDest, ((aSrc.OrderQty != 0m) ? (accrualAmt.Cury ?? 0m) * aDest.Qty / aSrc.OrderQty : (accrualAmt.Cury ?? 0m)) ?? 0m, CMPrecision.TRANCURY);
				aDest.TranUnitCost = (aSrc.OrderQty != 0m) ? (accrualAmt.Base ?? 0m) / aSrc.OrderQty : (accrualAmt.Base ?? 0m);
				aDest.CuryTranUnitCost = (aSrc.OrderQty != 0m) ? (accrualAmt.Cury ?? 0m) / aSrc.OrderQty : (accrualAmt.Cury ?? 0m);
			}
			else
			{
				...
				if (Document.Current.CuryID == company.Current.BaseCuryID)
				{
					...
					aDest.CuryTranUnitCost = aDest.TranUnitCost;
				}
				else //safety measure for the situation when user changed value of posetup.changeCuryRateOnReceipt and current receipt is in non-base currency
				{
					...
					decimal curyUnitCost, curyExtCost, curyTranCost, curyTranUnitCost;
					PXDBCurrencyAttribute.CuryConvCury(transactions.Cache, aDest, aDest.UnitCost ?? 0m, out curyUnitCost);
					...
					aDest.CuryUnitCost = curyExtCost;
					...
					PXDBCurrencyAttribute.CuryConvCury(transactions.Cache, aDest, aDest.TranUnitCost ?? 0m, out curyTranUnitCost);
