...
namespace PX.Objects.AM
{
    ...
    public class ECOMaint : PXGraph<ECOMaint, AMECOItem>
    {
        ...
        public ECOMaint()
        {
            ...
            ActionDropMenu.AddMenuAction(commitChanges);
        ...
        {
            IEnumerable<AMECOItem> items = adapter.Get<AMECOItem>().ToArray();

            Save.Press();

            foreach (AMECOItem item in items)
            {
                item.Hold = true;
                item.Approved = false;
                item.Rejected = false;
        ...
        [PXUIField(DisplayName = "Commit Changes to BOM", Visible = false, MapEnableRights = PXCacheRights.Select)]
        [PXButton]
        ...
        protected virtual void UpdateECRStatus(string ecoID, int newStatus)
        {
        ...
        {
            commitChanges.SetEnabled(enable);
        }
        ...
        public virtual void UpdateECRStatus(AMECRItem ecrItem, int newStatus)
        {
            if (ecrItem == null)
            {
                return;
        ...
        protected virtual void EnableECOItemFields(PXCache cache, AMECOItem item)
        {
            ...
            PXUIFieldAttribute.SetEnabled<AMECOItem.hold>(cache, item, !isCompleted);
        }
