...
#pragma warning restore 618
		...
		protected static List<fieldContainer> getAllNestedTypes(Type root, bool isExposedView, out fieldContainer tree)
		{
			List<fieldContainer> children = new List<fieldContainer>();
			List<fieldContainer> ret = new List<fieldContainer>();
			tree = new fieldContainer { Field = root };
			ret.Add(tree);
			foreach (Type child in root.GetNestedTypes())
			{
				if (isExposedView)
				{
					List<fieldContainer> subret = getAllNestedTypes(child, isExposedView, out var subtree);
		...
		public class FieldTree
		{
			...
			public FieldTree(fieldContainer root)
			{
				...
				Children = root.Children.Select(_ => new FieldTree(_)).ToArray();
			}
		}
		...
		internal protected static DACFieldDescriptor[] _GetProperties(Type type, List<Type> extensions, out Dictionary<string, List<PropertyInfo>> nameToFieldPropertyInfo, bool isExposedView, out int origLength, out fieldContainer tree)
		{
			...
			if (typeof(UI.PXExposedView).IsAssignableFrom(type))
			{
				...
				foreach (Type field in getAllNestedTypes(type, typeof(UI.PXExposedView).IsAssignableFrom(type), out tree).Select(_ => _.Field))
				{
					DACFieldDescriptor descr;
					...
					else if (field.BaseType.DeclaringType.DeclaringType == type.DeclaringType)
					{
