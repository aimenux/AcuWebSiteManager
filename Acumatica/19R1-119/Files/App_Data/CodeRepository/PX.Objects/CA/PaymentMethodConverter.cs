...
namespace PX.Objects.CA
{
	public class PaymentMethodConverter : PXGraph<PaymentMethodConverter>
	{
		...
		public PXSelect<CCProcessingCenter, Where<CCProcessingCenter.processingCenterID, Equal<Optional<Filter.oldCCProcessingCenterID>>>> OldProcessingCenters;
		...
		protected virtual void Filter_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
		{
			...
			if (allDataIsFilled)
			{
				...
				string oldProcCenterId = OldProcessingCenters.SelectSingle()?.ProcessingCenterID;
				string newProcCenterId = ProcessingCenters.SelectSingle()?.ProcessingCenterID;
			...
			ShowUnknownPluginWarningIfNeeded<Filter.oldCCProcessingCenterID>(row, OldProcessingCenters.Select(), row.OldCCProcessingCenterID);
			ShowUnknownPluginWarningIfNeeded<Filter.newCCProcessingCenterID>(row, ProcessingCenters.Select(), row.NewCCProcessingCenterID);
		}
		...
		private void ShowUnknownPluginWarningIfNeeded<T>(Filter row, CCProcessingCenter procCenter, string value) where T : IBqlField
		{
			if (procCenter != null)
			{
				if (procCenter != null && !IsKnownPlugun(procCenter.ProcessingTypeName))
				{
					filter.Cache.RaiseExceptionHandling<T>(row, value, new PXSetPropertyException(Messages.IncompatiblePluginForCardProcessing, PXErrorLevel.Warning));
				}
				else
				{
					filter.Cache.RaiseExceptionHandling<T>(row, value, null);
				}
			}
		}
	...
	public class PaymentMethodUpdater : PXGraph<PaymentMethodUpdater, CustomerPaymentMethod>
	{
		...
		{
			ConvertCustomerPaymentMethod(false, cpm, newCCPC);
		}
		...
		public void ConvertCustomerPaymentMethod(bool processExpiredCards, CustomerPaymentMethod cpm, CCProcessingCenter newCCPC)
		{
			...
			if (processExpiredCards && CheckCardIsExpired(cpm, out expiredDate) && !CheckProcCenterTokenizing(cpm.CCProcessingCenterID))
			{
		...
		private void DoConvert(CustomerPaymentMethod cpm, CCProcessingCenter newCCPC)
		{
			...
			bool doesOldProcCenterSupportTokenizing = CheckProcCenterTokenizing(oldCCProcessingCenterID);
			bool doesNewProcCenterSupportTokenizing = CheckProcCenterTokenizing(newCCPC.ProcessingCenterID);
		...
		private bool CheckProcCenterTokenizing(string procCenterId)
		{
