    ...
    public class EmployeeCostEngine
	{
		...
		public virtual decimal GetEmployeeHoursFromCalendar(int? employeeID)
		{
			...
			if (employee != null && employee.CalendarID != null)
			{
				...
				{
					hours = 0;
					if (calendar.MonWorkDay == true && calendar.MonEndTime.HasValue && calendar.MonStartTime.HasValue) hours += calendar.MonEndTime.Value.Subtract(calendar.MonStartTime.Value).TotalHours;
					if (calendar.TueWorkDay == true && calendar.TueEndTime.HasValue && calendar.TueStartTime.HasValue) hours += calendar.TueEndTime.Value.Subtract(calendar.TueStartTime.Value).TotalHours;
					if (calendar.WedWorkDay == true && calendar.WedEndTime.HasValue && calendar.WedStartTime.HasValue) hours += calendar.WedEndTime.Value.Subtract(calendar.WedStartTime.Value).TotalHours;
					if (calendar.ThuWorkDay == true && calendar.ThuEndTime.HasValue && calendar.ThuStartTime.HasValue) hours += calendar.ThuEndTime.Value.Subtract(calendar.ThuStartTime.Value).TotalHours;
					if (calendar.FriWorkDay == true && calendar.FriEndTime.HasValue && calendar.FriStartTime.HasValue) hours += calendar.FriEndTime.Value.Subtract(calendar.FriStartTime.Value).TotalHours;
					if (calendar.SatWorkDay == true && calendar.SatEndTime.HasValue && calendar.SatStartTime.HasValue) hours += calendar.SatEndTime.Value.Subtract(calendar.SatStartTime.Value).TotalHours;
					if (calendar.SunWorkDay == true && calendar.SunEndTime.HasValue && calendar.SunStartTime.HasValue) hours += calendar.SunEndTime.Value.Subtract(calendar.SunStartTime.Value).TotalHours;
				}
			}
