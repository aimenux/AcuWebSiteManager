...
namespace PX.Objects.TX
{
		...
		where TPrimary : class, IBqlTable, new()
	{
		...
		protected static void VerifyTransactions(string calcMode, IEnumerable<Tax> taxes)
		{
			...
			{
				if (taxes.Any(tax => tax.TaxCalcType == CSTaxCalcType.Doc && tax.TaxCalcLevel == CSTaxCalcLevel.CalcOnItemAmt))
				{
					throw new PXException(Messages.GrossModeIsNotAvailableForDocument, Messages.CalcRuleDocAmt);
				}

				if (taxes.Any(tax => tax.TaxCalcType == CSTaxCalcType.Doc && tax.TaxCalcLevel == CSTaxCalcLevel.CalcOnItemAmtPlusTaxAmt))
				{
					throw new PXException(Messages.GrossModeIsNotAvailableForDocument, Messages.CalcRuleDocAmtPlusTaxAmt);
				}
