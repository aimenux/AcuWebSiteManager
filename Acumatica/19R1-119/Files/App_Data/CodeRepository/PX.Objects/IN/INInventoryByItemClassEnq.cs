			...
			foreach (InventoryItem inventory in inventories)
			{
				if (needToDefault)
				{
					if (inventory.StkItem == true)
					{
						...
						using (stockItemMaint.Value.MakeRuleWeakeningScopeFor<InventoryItem.lotSerClassID>(RuleWeakenLevel.SuppressError))
						using (stockItemMaint.Value.MakeRuleWeakeningScopeFor<InventoryItem.baseUnit>(RuleWeakenLevel.SuppressError))
						{
							var copy = (InventoryItem)stockItemMaint.Value.Item.Cache.CreateCopy(inventory);
							copy.ItemClassID = newItemClassID;
							stockItemMaint.Value.Item.Update(copy);
						}
					...
					else
					{
						...
						using (nonStockItemMaint.Value.MakeRuleWeakeningScopeFor<InventoryItem.lotSerClassID>(RuleWeakenLevel.SuppressError))
						using (nonStockItemMaint.Value.MakeRuleWeakeningScopeFor<InventoryItem.baseUnit>(RuleWeakenLevel.SuppressError))
						{
							var copy = (InventoryItem)nonStockItemMaint.Value.Item.Cache.CreateCopy(inventory);
							copy.ItemClassID = newItemClassID;
							nonStockItemMaint.Value.Item.Update(copy);
						}
