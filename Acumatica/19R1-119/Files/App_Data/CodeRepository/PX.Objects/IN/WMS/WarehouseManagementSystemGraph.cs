...
namespace PX.Objects.IN
{
		...
		where TWMSHeader : WMSHeader, IBqlTable, new()
	{
		...
		private Dictionary<int, Dictionary<int, decimal>> _nonStockKitSpecs = new Dictionary<int, Dictionary<int, decimal>>();
		#endregion
		...
		protected virtual bool ProcessSingleBarcode(TWMSHeader doc)
		{
			...
			doc.InitialScanState = doc.ScanState;
		...
		protected bool ProcessEAN128Barcode(string barcode)
		{
			...
			void EnsureQty()
			{
				...
				if (decimalAI != null)
				{
					HeaderSetter.Set(x => x.Qty, ((DecimalData)segments[decimalAI]).Value);
			...
			where TArguments : WMSWaitArguments<TData>
		{
			...
			void safeMethod(object[] arguments)
			{
				...
				{
					var status = ex is PXBaseRedirectException 
						|| ex is PXReportRequiredException 
						|| !(ex is PXOperationCompletedSingleErrorException || ex is PXOperationCompletedWithErrorException)
		...
		private PXResult<INItemXRef, InventoryItem, INSubItem, INLotSerClass> ReadItemById(string barcode, INPrimaryAlternateType? additionalAlternateType = null)
		{
			...
			{
				var lotSerialClass = INLotSerClass.PK.Find(Base, inventory.LotSerClassID);
				if (lotSerialClass != null)
				{
					return new PXResult<INItemXRef, InventoryItem, INSubItem, INLotSerClass>(
						new INItemXRef { InventoryID = inventory.InventoryID, AlternateID = barcode },
						inventory,
						new INSubItem { },
						lotSerialClass);
				}
		...
		protected bool IsNonStockKit(int? inventoryID) => InventoryItem.PK.Find(Base, inventoryID).With(i => i.StkItem == false && i.KitItem == true);
		protected IReadOnlyDictionary<int, decimal> GetNonStockKitSpec(int kitInventoryID)
		{
			if (_nonStockKitSpecs.TryGetValue(kitInventoryID, out var nonStockKitSpec) == false)
			{
				var stockComponents =
					...
					.View.Select(Base, kitInventoryID)
					.AsEnumerable()
