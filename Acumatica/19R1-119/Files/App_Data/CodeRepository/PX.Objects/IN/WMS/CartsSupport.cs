		...
		where TScanLine : class, IBqlTable, new()
	{
		...
		protected virtual bool ValidateConfirmation(out WMSFlowStatus flowStatus)
		{
			...
			var needLotSerialNbr = IsLotSerialRequired
				&& CurrentHeader.LotSerTrack.IsNotIn(null, INLotSerTrack.NotNumbered)
		...
		protected virtual void ConfirmCartIn()
		{
			WMSFlowStatus Implementation()
			{
				...
				Report(Msg.InventoryAdded, LinesView.Cache.GetValueExt<ScanLine.inventoryID>(existLine), header.Qty);
		...
		protected virtual void ConfirmCartOut()
		{
			WMSFlowStatus Implementation()
			{
				...
				Report(Msg.InventoryAdded, LinesView.Cache.GetValueExt<ScanLine.inventoryID>(existLine), confirmQty);
