...
namespace PX.Objects.IN
{
	...
	public class INUnitAttribute : AcctSub2Attribute, IPXFieldVerifyingSubscriber, IPXRowSelectedSubscriber, IPXRowPersistingSubscriber
	{
		...
		{
			if(_BaseUnitType == null)
				return null;
			string unit = (string)GetSelectorParameterValue(cache, data, _BaseUnitType);
			if(unit == null)
			{
				var itemType = BqlCommand.GetItemType(_BaseUnitType);
				var itemCache = cache.Graph.Caches[itemType];
				if(itemCache.Keys.Count == 0)
				{
					var cmd = BqlTemplate.OfCommand<Select<BqlPlaceholder1>>.Replace<BqlPlaceholder1>(itemType).ToCommand();
					var view = cache.Graph.TypedViews.GetView(cmd, true);
					var item = view.SelectSingle();
					unit = (string)itemCache.GetValue(item, _BaseUnitType.Name);
				}
			}
			return unit;
		}
