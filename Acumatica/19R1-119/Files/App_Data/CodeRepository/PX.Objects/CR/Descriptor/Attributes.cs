...
namespace PX.Objects.CR
{
	...
	public sealed class CRDuplicateBAccountSelectorAttribute : PXCustomSelectorAttribute
	{
			...
			View = new PXView(_Graph, false,
				new Search2<
					...
					InnerJoin<CRGrams2,
						...
						And<CRGrams.entityID, Equal<Current<BAccount.defContactID>>>>>>,
					InnerJoin<BAccountR,
						On<BAccountR.defContactID, Equal<CRGrams2.entityID>>,
					...
					Where<
						CRGrams.entityID, Equal<Current<BAccount.defContactID>>,
						And<CRGrams.validationType, Equal<ValidationTypesAttribute.account>>>>(),
		...
		private IEnumerable Handler()
		{
			...
			PXView.StartRow = 0;
			foreach (PXResult<CRGrams, CRGrams2, BAccountR, CRLeadContactValidationProcess.Contact2> rec in list)
			{
				CRGrams db = rec;
				CRGrams2 db2 = rec;

				if (db.Score < currentSetup.AccountValidationThreshold)
				{
					continue;
		}
				...
				yield return new PXResult<CRDuplicateRecordVirtual, BAccountR, CRLeadContactValidationProcess.Contact2>(
					dupRecord,
					rec,
					rec);
			}
		...
		public IEnumerable GetRecords()
		{
			...
			foreach (PXResult<CRDuplicateRecordVirtual, BAccountR,CRLeadContactValidationProcess.Contact2> result in View.SelectMulti(currentID))
			{
				BAccountR b = result;
				CRDuplicateRecordVirtual d = result;
				d = (CRDuplicateRecordVirtual)_Graph.Caches[typeof(CRDuplicateRecordVirtual)].Locate(d);
