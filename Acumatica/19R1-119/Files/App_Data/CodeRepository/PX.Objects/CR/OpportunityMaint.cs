	...
	public class OpportunityMaint : PXGraph<OpportunityMaint>, PXImportAttribute.IPXPrepareItems, IGraphWithInitialization
	{
        ...
        protected virtual void DoCreateSalesOrder(CreateSalesOrderFilter param)
        {
			...
			if (_CRContact != null)
			{
				...
				if (_billingContact != null)
				{
					_CRContact.RevisionID = _CRContact.RevisionID ?? _billingContact.RevisionID;
					if (_billingContact.RevisionID != _CRContact.RevisionID)
					{
						_CRContact.IsDefaultContact = false;
					}
				}
				ContactAttribute.CopyRecord<SOOrder.billContactID>(docgraph.Document.Cache, doc, _CRContact, true);
			}
			...
			if (_CRAddress != null)
			{
				...
				if (_billingAddress != null)
				{
					_CRAddress.RevisionID = _CRAddress.RevisionID ?? _billingAddress.RevisionID;
					if (_billingAddress.RevisionID != _CRAddress.RevisionID)
					{
						_CRAddress.IsDefaultAddress = false;
					}
				}
				AddressAttribute.CopyRecord<SOOrder.billAddressID>(docgraph.Document.Cache, doc, _CRAddress, true);
			}
