	...
	public class SOInvoiceEntry : ARInvoiceEntry
	{
		...
		public override IEnumerable Release(PXAdapter adapter)
		{
			...
			PXLongOperation.StartOperation(this, delegate ()
			{
				...
				PXNoteAttribute.ForcePassThrow<ARTran.noteID>(ie.Freight.Cache);
				...
				try
				{
					var lastCcTransByInvoices = new Dictionary<ARDocKey, ICCPaymentTransaction>();
					foreach (ARRegister ardoc in list)
					{
						var ccProcInvoiceTrans = CCProcTranHelper.GetSOInvoiceCCProcTrans(this, (ARInvoice) ardoc); 
						var ccPTran = CCProcTranHelper.FindCCLastSuccessfulTran(ccProcInvoiceTrans);
						lastCcTransByInvoices.Add(new ARDocKey(ardoc.DocType, ardoc.RefNbr), ccPTran);
					}
					...
					ARDocumentRelease.ReleaseDoc(list, adapter.MassProcess, null, null, delegate (ARRegister ardoc)
					{
						...
						ICCPaymentTransaction ccPTran;
						lastCcTransByInvoices.TryGetValue(new ARDocKey(ardoc.DocType, ardoc.RefNbr), out ccPTran);
						...
						ie.CopyFreightNotesAndFilesToARTran(ardoc);
					});
				}
				finally
				{
		...
		protected override void ARInvoice_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
            ...
            bool releaseActionEnabled = !enableCCProcess || arsetup.Current.IntegratedCCProcessing != true || (doc.DocType == ARDocType.CashReturn ? isCCRefunded : isCCCaptured) || !CCProcTranHelper.HasSuccessfulCCTrans(ccProcTran);
		...
		public virtual IEnumerable sHipmentlist()
		{
			...
			foreach (PXResult<SOOrderShipment, SOOrder, SOShipment, SOOrderType> order in PXSelectJoin<SOOrderShipment,
				...
				Where<SOOrderShipment.customerID, Equal<Current<ARInvoice.customerID>>,
					...
					And<Where<SOOrderShipment.invoiceNbr, IsNull,
						Or<SOOrderShipment.invoiceNbr, Equal<Current<ARInvoice.refNbr>>>>>>>>>>.Select(this))
			{
				if (cmd.View.SelectSingleBound(new object[] { (SOOrderShipment)order }) != null)
					continue;

				if (newInvoice || list.Find<ARInvoice.customerID, SOInvoice.billAddressID, SOInvoice.billContactID, ARInvoice.curyID, ARInvoice.termsID, ARInvoice.hidden>(
						((SOOrder)order).CustomerID, ((SOOrder)order).BillAddressID, ((SOOrder)order).BillContactID, ((SOOrder)order).CuryID, ((SOOrder)order).TermsID, false) != null)
				{
					selectedShipments.Remove((SOOrderShipment)order);
					yield return (SOOrderShipment)order;
				}
			}

			foreach (PXResult<SOOrderShipment, SOOrder, POReceipt, SOOrderType> order in PXSelectJoin<SOOrderShipment,
		...
		public virtual void InvoiceOrder(DateTime invoiceDate, PXResult<SOOrderShipment, SOOrder, CurrencyInfo, SOAddress, SOContact> order, PXResultset<SOShipLine, SOLine> details, Customer customer, DocumentList<ARInvoice, SOInvoice> list, PXQuickProcess.ActionFlow quickProcessFlow)
		{
			...
			if (CCProcTranHelper.IsOrderSelfCaptured(this, soOrder))
			{
		...
		public virtual void CopyFreightNotesAndFilesToARTran(ARRegister doc)
		{
			...
			if (!freights.Any())
				return;
			var freightLines = PXSelect<ARTran,
					Where<ARTran.tranType, Equal<Required<ARInvoice.docType>>,
					And<ARTran.refNbr, Equal<Required<ARInvoice.refNbr>>,
					And<ARTran.lineType, Equal<SOLineType.freight>,
					And<ARTran.sOShipmentType, NotEqual<SOShipmentType.dropShip>>>>>>.Select(this, doc.DocType, doc.RefNbr).RowCast<ARTran>().ToLookup(x => x.SOShipmentNbr);
			foreach (var freight in freights)
				foreach (var line in freightLines[freight.ShipmentNbr])
		...
		public class PaymentTransaction : PaymentTransactionGraph<ARInvoiceEntry, ARInvoice>
		{
			...
			public static void UpdateSOInvoiceState(IBqlTable aDoc, CCTranType lastOperation, bool success)
			{
				...
				if (string.IsNullOrEmpty(doc.ExtRefNbr) && doc.IsCCCaptured == true && currTran != null)
				{
					doc.ExtRefNbr = currTran.PCTranNumber;
				}
				if (doc.IsCCCaptured == false && currTran != null)
				{
