...
namespace PX.Objects.Common.Discount
{
	public abstract class DiscountEngine : PXGraph<DiscountEngine>
	{
		...
		[Obsolete(Messages.MethodIsObsoleteAndWillBeRemoved2020R1)]
		protected ARSalesPrice GetARPrice(PXCache cache,
			int inventoryID, int customerID, string customerPriceClassID, string curyID, string UOM, decimal quantity, DateTime date)
		{
			var salesPrice = GetSalesPrice(cache, inventoryID, null, customerID, customerPriceClassID, curyID, UOM, quantity, date, false);
				...
				: new ARSalesPrice
					{
						...
						CuryID = salesPrice.CuryID,
						PriceType = salesPrice.PriceType,
						IsPromotionalPrice = salesPrice.IsPromotionalPrice,
						IsProrated = salesPrice.Prorated
					};
		}
