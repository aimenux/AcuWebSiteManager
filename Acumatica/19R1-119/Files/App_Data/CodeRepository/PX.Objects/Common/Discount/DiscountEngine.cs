		...
		protected virtual void ApplyDiscountToLine(PXCache sender, TLine line, decimal? qty, decimal? curyUnitPrice, decimal? curyExtPrice, DiscountLineFields dLine, DiscountResult discountResult, int multInv)
		{
			...
			if (!discountResult.IsEmpty)
			{
				...
				else
				{
					...
					bool isOriginalExtPrice = curyExtPrice == PXCurrencyAttribute.Round(cache, row, priceSign * (curyUnitPrice ?? 0) * qtyVal, CMPrecision.TRANCURY);
					if (lineDiscountTarget == AR.LineDiscountTargetType.SalesPrice && !isOriginalExtPrice)
					{
						decimal salesPriceAfterDiscount = priceSign * (curyUnitPrice ?? 0) * (1 - 0.01m * (dLine.DiscPct ?? 0));
