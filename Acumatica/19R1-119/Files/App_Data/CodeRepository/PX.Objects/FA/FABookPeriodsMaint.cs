...
namespace PX.Objects.FA
{
	public class FABookPeriodsMaint : PXGraph<FABookPeriodsMaint>
	{
		...
		protected virtual IEnumerable first(PXAdapter adapter)
		{
			...
			FillCurrentOfBookYear(bookYear);
		...
		protected void FillCurrentOfBookYear(FABookYear bookYear)
		{
			FABookYear current = BookYear.Cache.Current as FABookYear;

			current.Year = bookYear?.Year;
			current.StartDate = bookYear?.StartDate;
			current.FinPeriods = bookYear?.FinPeriods;
		}
		...
		protected IEnumerable bookYear()
		{
			if (BookYear.Cache.InternalCurrent == null)
			{
				...
				BookYear.Cache.Current = defaultYear ?? throw new PXSetupNotEnteredException<FABookYear>(Messages.NoCalendarDefined);
			...
			else if (currentFABookYear.Year == null)
			{
