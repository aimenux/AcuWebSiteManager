							...
							if (isFirstAPTaxRow)
						{
							...
							if (poRes != null)
							{
								...
								if (_IsIntegrityCheck == false && updatePOOrder && srcLine.POAccrualType != null)
								{
									...
									if (updLine.Closed == false)
									{
										decimal? unbilledQty = ((updLine.OrderQty < updLine.CompletedQty) ? updLine.CompletedQty : updLine.OrderQty) - updLine.BilledQty;
										updLine.UnbilledQty = (unbilledQty < 0m) ? 0m : unbilledQty;
										decimal? curyUnbilledAmt = updLine.CuryExtCost + updLine.CuryRetainageAmt - updLine.CuryBilledAmt;
										updLine.CuryUnbilledAmt = (curyUnbilledAmt < 0m) ? 0m : curyUnbilledAmt;
									}
