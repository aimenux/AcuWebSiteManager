	...
	public class APDocumentEnq : PXGraph<APDocumentEnq>
	{
		...
		protected virtual IEnumerable documents()
		{
			...
			foreach (PXResult<APDocumentResult, Sub, APInvoice, APPayment> reg in sel.Select(branchIDs)
					/*
					sel.Select(branchIDs).Select(r => new
					{						
					 Invoice = new
					  {
						  r.GetItem<APInvoice>().DocType,
						  r.GetItem<APInvoice>().RefNbr,
						  r.GetItem<APInvoice>().InvoiceNbr,
						  r.GetItem<APInvoice>().InstallmentCntr,
						  r.GetItem<APInvoice>().SuppliedByVendorID,
					  },
					  Payment = new
					  {
						  r.GetItem<APPayment>().DocType,
						  r.GetItem<APPayment>().RefNbr,
						  r.GetItem<APPayment>().PaymentMethodID,
						  r.GetItem<APPayment>().ExtRefNbr,					  
					  },
					  Document = r.GetItem<APDocumentResult>(),
					})
					*/)
			{
