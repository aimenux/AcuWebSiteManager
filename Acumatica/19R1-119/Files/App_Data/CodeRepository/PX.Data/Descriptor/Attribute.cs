...
namespace PX.Data
{
	...
	/// <summary>
	/// Attribute indicates that field input should be restricted by dynamic mask <see cref="BqlCommand"/> specified in constructor.
	/// For UI it restricts from put symbols that doesn't match mask,
	/// for Contact-Based API it could be disabled, enabled for field selecting, or checked during row persisting,
	/// to change this behavior specify <see cref="CBApiValidationType"/> property.
	/// </summary>
	/// <example>
	/// [PXDBString(20)]
	/// [PXUIField(DisplayName = "Postal Code")]
	...
	public class PXDynamicMaskAttribute : PXEventSubscriberAttribute, IPXFieldSelectingSubscriber, IPXRowPersistingSubscriber
	{
		...
		/// <summary>
		/// Indicates how mask validation should work for Contact-Based API import
		/// (<see cref="PXGraph.IsContractBasedAPI"/> is set to <see langword="true"/>)
		/// or for copy paste context (<see cref="PXGraph.IsCopyPasteContext"/> is set to <see langword="true"/>).
		/// </summary>
		public enum ValidationType
			{
			/// <summary>
			/// Mask validation doesn't work.
			/// </summary>
			None = 0,
			/// <summary>
			/// Mask validation works during <see cref="IPXFieldSelectingSubscriber.FieldSelecting(PXCache, PXFieldSelectingEventArgs)"/>.
			/// </summary>
			FieldSelecting = 1,
			/// <summary>
			/// Mask validation works during <see cref="IPXRowPersistingSubscriber.RowPersisting(PXCache, PXRowPersistingEventArgs)"/>.
			/// </summary>
			RowPersisting = 2,
			}
		...
		/// <summary>
		/// Get or set default mask that will be used, if dynamic mask would return nothing.
		/// The default value is <see cref="string.Empty"/> which means the mask won't be applied.
		/// </summary>
		...
		/// Indicates how mask validation should work for Contact-Based API import.
		/// (<see cref="PXGraph.IsContractBasedAPI"/> is set to <see langword="true"/>).
		/// Default value is <see cref="ValidationType.RowPersisting"/>.
		/// </summary>
		public ValidationType CBApiValidationType { get; set; } = ValidationType.RowPersisting;

		/// <summary>
		/// Indicates how mask validation should work for
		/// copy paste context (<see cref="PXGraph.IsCopyPasteContext"/> is set to <see langword="true"/>).
		/// Default value is <see cref="ValidationType.None"/>.
		/// </summary>
		...
		/// <summary>
		/// Try get mask for specified field. Return found dynamic mask or <see cref="DefaultMask"/>.
		/// </summary>
		/// <param name="graph"></param>
		/// <param name="row"></param>
		/// <param name="mask">Found validation mask, or <see cref="DefaultMask"/>, depending on returned value.</param>
		/// <returns><see langword="true"/> if mask found,
		/// otherwise return <see langword="false"/>, which means than <see cref="DefaultMask"/> is used.</returns>
